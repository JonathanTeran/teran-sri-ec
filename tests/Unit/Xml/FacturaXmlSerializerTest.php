...
class FacturaXmlSerializerTest extends TestCase
{
    ...
    public function test_tarifa_defaults_to_zero_formatted_when_absent(): void
    {
        // Build a Factura whose detail impuesto has NO 'tarifa' key.
        // Impuesto::fromArray leaves tarifa = null when the key is absent.
        $factura = Factura::fromArray([
            ...
            'detalles' => [
                [
                    'codigoPrincipal' => 'PROD001',
                    'descripcion' => 'Producto sin tarifa',
                    'cantidad' => '1.00',
                    'precioUnitario' => '100.00',
                    'descuento' => '0.00',
                    'precioTotalSinImpuesto' => '100.00',
                    'impuestos' => [
                        // Note: 'tarifa' key deliberately omitted → tarifa = null
                        ['codigo' => '2', 'codigoPorcentaje' => '4', 'baseImponible' => '100.00', 'valor' => '15.00'],
                    ],
                ],
            ],
        ...
        $clave = '2601202601179001100100110010010000000011234567819';
        $xml = (new FacturaXmlSerializer())->serialize($factura, $clave);

        // Must emit the decimal-formatted default, not an empty element.
        $this->assertStringContainsString('<tarifa>0.00</tarifa>', $xml);

        // Must still pass XSD validation.
        $xsdPath = __DIR__ . '/../../../resources/xsd/factura_v2.1.0.xsd';
        $this->assertTrue(XsdValidator::validate($xml, $xsdPath));
    }
}
